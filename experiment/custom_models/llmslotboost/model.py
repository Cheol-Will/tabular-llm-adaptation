...
from ..layer import FeatureTokenizer, MultiOutputProj
...
class LLMSlotBoost(nn.Module):
    ...
    def __init__(
        ...
        num_boosting: int = None, # num_boosting 
        boosting_token: str = 'eos', # 'eos' | 'trainable'
    ):
        super().__init__()
        assert attn_type in ('causal', 'bidir', 'structured')
        assert prediction_method in ('next_token_pred', 'token_pooling')
        assert boosting_token in ('eos', 'trainable')
        ...
        self.backbone = AutoModelForCausalLM.from_pretrained(
            local_model_path, dtype=torch.bfloat16,
        ...
        self.output_proj = MultiOutputProj(self.llm_dim, num_classes, mlp_ratio, num_boosting)
        
        if boosting_token == 'trainable':
            self.boosting_token = nn.Parameter(torch.empty(1, num_boosting-1, self.llm_dim))
            nn.init.uniform_(self.boosting_token)
        else:
            self.boosting_token = None
    ...
    def forward(
        ...
        with autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if self.boosting_token is not None:
                boosting_token = self.boosting_token.expand(B, -1, -1)
            else:
                eos_token = inputs_embeds[:, -1, :].unsqueeze(1)
                boosting_token = eos_token.expand(-1, self.num_boosting - 1, -1)

            inputs_embeds = torch.cat([inputs_embeds, boosting_token], dim=1)
            ...
            if self.prediction_method == 'next_token_pred':
                pred_hidden = hidden_state[:, -self.num_boosting:, :] # (B, N, D)
            else:  # token_pooling
